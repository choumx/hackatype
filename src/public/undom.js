...
function undom() {
  ...
  class Node {
    ...
    propertyIsInherited(property) {
      return (
        ["nodeType", "nodeName", "childNodes", "parentNode"].indexOf(
          property
        ) >= 0
      );
    }
  ...
  class Text extends Node {
    ...
    propertyIsInherited(property) {
      return (
        super.propertyIsInherited(property) || ["data"].indexOf(property) >= 0
      );
    }
    get textContent() {
      return this.data;
  ...
  class Element extends Node {
    ...
    propertyIsInherited(property) {
      return (
        super.propertyIsInherited(property) ||
        ["attributes", "children", "style"].indexOf(property) >= 0
      );
    }
  ...
  function isDOMProperty(target, value) {
    ...
    if (PREACT_PROPS[value]) {
      // TODO(willchou): Replace this with something better.
      return false;
    }
    if (!target.propertyIsEnumerable(value)) {
      return false;
    }
    if (target.propertyIsInherited(value)) {
      // Skip Node.nodeType etc.
      return false;
    }
    return true;
  }
